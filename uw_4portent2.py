...
count_island = 0 #counts how many times we have BASIC ISLANDS + 1 cantrip in our hand.
count_four_plus = 0 #counts how many times we have 4+ landers i.e, flooding
...
for _ in range(num_iterations):
    ...
    count_island += (min(draw['Islands'], 1) + min(draw['Cantrips'], 1) >= 2) # minimum of 1 Basic Island AND at least 1 Cantrip
    count_four_plus += (draw['Islands'] + draw['Blue_Fetches'] + draw['Plains'] + draw['Arid_Mesa'] + draw['Tundra'] >= 4) # drawing a 4 lander in the 7
...
print()
print('Amount of times we hit BASIC ISLAND + 1 cantrip:             \t' + str(round(count_island / num_iterations,3)))
print('Amount of 4 land openers:                                    \t' + str(round(count_four_plus / num_iterations,3)))
