...
decklist = {'Plains': 2, 'Islands': 6, 'Blue_Fetches': 8, 'Arid_Mesa': 1, 'Tundra': 2, 'Cantrips':11, 'Others': 30} #we take out karakas in this build and replace with +1 cantrip. Also -1 spell +1 cantrip for the Full Cartel of 12 Cantrips.
deck = []
...
print('Amount of times we hit Non-Wastable blue source + 1 cantrip:     \t' + str(round((cantrip_only / num_iterations) * (good_lands_only / num_iterations) ,3))) # need to check logic in the loop
print('Amount of times we hit any blue source + 1 cantrip:              \t' + str(round((cantrip_only / num_iterations) * (count_all_lands / num_iterations) ,3))) # need to check logic in the loop
